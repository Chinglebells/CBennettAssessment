...
def remove_punct(text):
    ...
    'goSouTh'
    """
    out = "".join(c for c in text if c.isalpha() == True or c == " ")
    return out     
...
def menu(exits):
    ...
    while True:
        ...
        # Read player's input
        # Normalise the input
        normalised = normalise_input(choice)
        # Check if the input makes sense (is valid exit)
        if is_valid_exit(exits,normalised) == True:         
            # If so, return the player's choice
            return normalised
        else:
            print("\nYou can't do that! Please type a direction such as 'east'.\n")
